...
def validar_email(email: str) -> str:
    ...
    if len(email) > constants.EMAIL_MAX_LENGTH:
        raise ValueError(f"Email muito longo (máximo {constants.EMAIL_MAX_LENGTH} caracteres)")
...
def validar_numero_aula(numero_aula: Optional[int]) -> Optional[int]:
    ...
    if not isinstance(numero_aula, int):
        raise ValueError(f"Número de aula deve ser inteiro")
    
    if numero_aula < constants.NUMERO_AULA_MIN or numero_aula > constants.NUMERO_AULA_MAX:
        raise ValueError(
...
def extrair_ra_usuario(usuario) -> str:
    ...
    raise ValueError("Não foi possível extrair RA do usuário: usuário sem atributo 'ra'")
