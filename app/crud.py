...
def criar_disciplina(db: Session, disciplina: schemas.DisciplinaCreate) -> models.Disciplina:
...
def obter_disciplina(db: Session, id_disciplina: int) -> Optional[models.Disciplina]:
    """Obter disciplina por ID."""
    return db.query(models.Disciplina).filter(models.Disciplina.id_disciplina == id_disciplina).first()


def obter_disciplinas(db: Session, skip: int = 0, limit: int = 100) -> List[models.Disciplina]:
    """Listar todas as disciplinas."""
    return db.query(models.Disciplina).offset(skip).limit(limit).all()


def atualizar_disciplina(
    db: Session, id_disciplina: int, disciplina: schemas.DisciplinaCreate
) -> Optional[models.Disciplina]:
    """Atualizar disciplina."""
    db_disciplina = obter_disciplina(db, id_disciplina)
    if db_disciplina:
...
def deletar_disciplina(db: Session, id_disciplina: int) -> bool:
    """Deletar disciplina."""
    db_disciplina = obter_disciplina(db, id_disciplina)
    if db_disciplina:
...
# ============================================================================
# CURSO-DISCIPLINA (Associativo)
# ============================================================================

def criar_curso_disciplina(
    db: Session, curso_disciplina: schemas.CursoDisciplinaCreate
) -> models.CursoDisciplina:
    """Criar associação curso-disciplina."""
    try:
        db_curso_disciplina = models.CursoDisciplina(**curso_disciplina.model_dump())
        db.add(db_curso_disciplina)
        db.commit()
        db.refresh(db_curso_disciplina)
        return db_curso_disciplina
    except IntegrityError:
        db.rollback()
        raise


def obter_disciplinas_por_curso(
    db: Session, id_curso: int, skip: int = 0, limit: int = 100
) -> List[models.CursoDisciplina]:
    """Listar disciplinas de um curso."""
    return (
        db.query(models.CursoDisciplina)
        .filter(models.CursoDisciplina.id_curso == id_curso)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def obter_disciplinas_por_curso_modulo(
    db: Session, id_curso: int, modulo: int, skip: int = 0, limit: int = 100
) -> List[models.CursoDisciplina]:
    """Listar disciplinas de um curso em um módulo específico."""
    return (
        db.query(models.CursoDisciplina)
        .filter(models.CursoDisciplina.id_curso == id_curso, models.CursoDisciplina.modulo == modulo)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def deletar_curso_disciplina(db: Session, id_curso: int, id_disciplina: int) -> bool:
    ...
    if db_curso_disciplina:
        db.delete(db_curso_disciplina)
        db.commit()
        return True
    return False
...
# ============================================================================
# DISCIPLINA-DOCENTE (Associativo)
# ============================================================================

def criar_disciplina_docente(
    db: Session, disciplina_docente: schemas.DisciplinaDocenteCreate
) -> models.DisciplinaDocente:
    """Criar associação disciplina-docente."""
    try:
        db_disciplina_docente = models.DisciplinaDocente(**disciplina_docente.model_dump())
        db.add(db_disciplina_docente)
        db.commit()
        db.refresh(db_disciplina_docente)
        return db_disciplina_docente
    except IntegrityError:
        db.rollback()
        raise


def obter_docentes_por_disciplina(
    db: Session, id_disciplina: int, skip: int = 0, limit: int = 100
) -> List[models.DisciplinaDocente]:
    """Listar docentes de uma disciplina."""
    return (
        db.query(models.DisciplinaDocente)
        .filter(models.DisciplinaDocente.id_disciplina == id_disciplina)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def deletar_disciplina_docente(db: Session, id_disciplina: int, id_docente: int) -> bool:
    """Deletar associação disciplina-docente."""
    db_disciplina_docente = (
        db.query(models.DisciplinaDocente)
        .filter(
    ...
    if db_disciplina_docente:
        db.delete(db_disciplina_docente)
        db.commit()
        return True
    return False
